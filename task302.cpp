...
void spawnPoisonFood() {
    poisonX = (rand() % (SCREEN_WIDTH / SNAKE_BLOCK_SIZE)) * SNAKE_BLOCK_SIZE;
    poisonY = (rand() % (SCREEN_HEIGHT / SNAKE_BLOCK_SIZE)) * SNAKE_BLOCK_SIZE;
    isPoisonActive = true;
    poisonStartTime = SDL_GetTicks(); 
}
...
void updateGame() {
    ...
    if (isPoisonActive && snake[0].x == poisonX && snake[0].y == poisonY) {
        score -= 10;
        isPoisonActive = false; 
        if (score < 0) {
            gameState = GAME_OVER;
    ...
    if (isPoisonActive && SDL_GetTicks() - poisonStartTime > 4000) {
        isPoisonActive = false;
    }
...
void renderGame() {
    ...
    if (isPoisonActive) {
        SDL_SetRenderDrawColor(renderer, 0, 0, 255, 255);
        SDL_Rect poisonRect = {poisonX, poisonY, SNAKE_BLOCK_SIZE, SNAKE_BLOCK_SIZE};
